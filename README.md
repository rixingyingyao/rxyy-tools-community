...
- **Codex**：原生任务绑定、增量输出、状态、问题与回答、新任务的模型/思考选择、在原任务接续。桌面和手机可投递图片与文件，运行中追加、结束后接续，并保留发送回执；未知回执不会自动重发。Windows 桌面 IPC 适配会核验进程和协议版本；协议不匹配时显示错误。
...
核心适配测试位于 `rxyy_mcp/tests`；社区安装/隔离/启动测试位于 `tests`，CI 列出社区版适用的回归套件。完整工作台的业务看板测试依赖未导出的业务模块，不能在社区版中全量发现运行。集成测试使用临时数据和端口，不发模型消息，不改真实任务。桌面协议随 Codex、Cursor 更新可能变化；请提交应用版本、错误信息和脱敏日志，勿上传聊天数据库或登录文件。
